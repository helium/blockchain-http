...
init({batch, Queries}) ->
    lists:foldl(
        fun({SQL, Params, Types}, #eequery{sql = SQLs, param_types = Typess, params = Paramss}) ->
            #eequery{
                sql = [SQL | SQLs],
                param_types = [Types | Typess],
                params = [Params | Paramss]
            }
        end,
        #eequery{sql = [], param_types = [], params = []},
...
execute(Sock, #eequery{sql = Sqls, param_types = ParamTypess, params = Paramss} = St) ->
    ...
    Commands = lists:flatmap(
        ...
        lists:zip3(Sqls, ParamTypess, Paramss)
    ),
...
handle_message(
    ?DATA_ROW,
    <<_Count:?int16, Bin/binary>>,
    Sock,
    #eequery{decoder = Decoder} = St
) ->
