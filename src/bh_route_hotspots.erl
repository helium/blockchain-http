...
handle('GET', [Address, <<"roles">>], Req) ->
    Args = ?GET_ARGS([cursor, max_time, min_time, limit, filter_types], Req),
    Result = bh_route_txns:get_role_list({hotspot, Address}, Args),
    CacheTime = bh_route_txns:get_txn_list_cache_time(Result),
    ?MK_RESPONSE(Result, CacheTime);
handle('GET', [Address, <<"roles">>, <<"count">>], Req) ->
    Args = ?GET_ARGS([filter_types], Req),
    ?MK_RESPONSE(bh_route_txns:get_role_count({hotspot, Address}, Args), block_time);
...
get_hotspot_list([
    ...
    Modes = ?HOTSPOT_MODES_TO_LIST(?HOTSPOT_MODES, undefined),
    Result = ?PREPARED_QUERY(?S_HOTSPOT_LIST, [Modes]),
...
get_hotspot_list([
    ...
    Modes = ?HOTSPOT_MODES_TO_LIST(?HOTSPOT_MODES, FilterModes),
    Result = ?PREPARED_QUERY(?S_HOTSPOT_LIST, [Modes]),
    mk_hotspot_list_from_result(
        #{filter_modes => Modes},
        Result
...
get_hotspot_list([
    ...
    Modes = ?HOTSPOT_MODES_TO_LIST(?HOTSPOT_MODES, FilterModes),
    Result = ?PREPARED_QUERY(?S_OWNER_HOTSPOT_LIST, [Owner, Modes]),
    mk_hotspot_list_from_result(
        #{filter_modes => Modes},
        Result
    );
get_hotspot_list([
    ...
    Modes = ?HOTSPOT_MODES_TO_LIST(?HOTSPOT_MODES, FilterModes),
    Result = ?PREPARED_QUERY(?S_CITY_HOTSPOT_LIST, [City, Modes]),
    mk_hotspot_list_from_result(
        #{filter_modes => Modes},
        Result
    );
get_hotspot_list([
    ...
    case ?CURSOR_DECODE(Cursor) of
        {ok,
            ...
            FilterModes = ?HOTSPOT_MODES_TO_LIST(
                ?HOTSPOT_MODES, maps:get(<<"filter_modes">>, C, ?HOTSPOT_MODES)
            ),
            case {Owner, City} of
                {undefined, undefined} ->
                    Result =
