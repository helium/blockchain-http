...
get_validator_list([{cursor, Cursor}]) ->
    case ?CURSOR_DECODE(Cursor) of
        {ok, #{
            ...
            Result =
                ?PREPARED_QUERY(?S_VALIDATOR_LIST_BEFORE, [BeforeAddress, BeforeBlock]),
            mk_validator_list_from_result(Result);
        _ ->
            {error, badarg}
    end.
...
mk_cursor(Results) when is_list(Results) ->
    case length(Results) < ?VALIDATOR_LIST_LIMIT of
        true ->
            undefined;
        false ->
            {Height, Address, _Owner, _Stake, _LastHeartbeat, _VersionHeartBeat, _Nonce,
...
validator_to_json(
    {Height, Address, Owner, Stake, LastHeartbeat, VersionHeartbeat, Nonce, _FirstBlock}
) ->
    #{
        <<"address">> => Address,
        <<"owner">> => Owner,
...
validator_to_json(
    {Height, Address, Owner, Stake, LastHeartbeat, VersionHeartbeat, Nonce, FirstBlock,
        SpecNonce}
) ->
